...
def clear_screen(screen):
    ...
    # The main function would use the local function play_game to 
    # run the game, and then draw the result of the game using 
    # a function implemented in the draw module called draw_game. 
    # To use the function draw_game from the draw module, 
    # we would need to specify in which module the function is implemented, 
    # using the dot operator. To reference the draw_game function 
    # from the game module, we would need to import the draw module 
    # and only then call draw.draw_game().

    # When the import draw directive will run, the Python interpreter 
    # will look for a file in the directory which the script was 
    # executed from, by the name of the module with a .py prefix, 
    # so in our case it will try to look for draw.py. 
    # If it will find one, it will import it. If not, 
    ...
    # Python compiles files into Python bytecode so that 
    # it won't have to parse the files each time modules are loaded. 
    # If a .pyc file exists, it gets loaded instead of the .py file, 
    # but this process is transparent to the user.

# Importing module objects to the current namespace
    # We may also import the function draw_game directly into the 
    # main script's namespace, by using the from command.
...
def main():
    ...
    # You may have noticed that in this example, draw_game does not 
    # precede with the name of the module it is imported from, 
    # because we've specified the module name in the import command.

    # The advantages of using this notation is that it is easier to 
    # use the functions inside the current module because you don't 
    # need to specify which module the function comes from. 
    # However, any namespace cannot have two objects with the exact 
    # same name, so the import command may replace an existing object 
    # in the namespace.

# Importing all objects from a module
    # We may also use the import * command to import all objects 
...
def main():
    ...
    # This might be a bit risky as changes in the module might affect
    #  the module which imports it, but it is shorter and also does 
    # not require you to specify which objects you wish to import 
    # from the module.

# Custom import name
    # We may also load modules under any name we want. This is useful 
    # when we want to import a module conditionally to use the same 
...
PYTHONPATH=/foo python game.py
    ...
    # This will execute game.py, and will enable the script to load 
...
sys.path.append("/foo")
    ...
    # This will add the foo directory to the list of paths to look 
    # for modules in as well.

# Exploring built-in modules
    # Check out the full list of built-in modules in the Python standard library 
    # here at https://docs.python.org/3/library/

    # Two very important functions come in handy when exploring modules 
    # in Python - the dir and help functions.

    # If we want to import the module urllib, which enables us to create 
    # read data from URLs, we simply import the module:
...
# Writing packages
    ...
    # We also must not forget to add the __init__.py file inside 
    # the foo directory.
...
from foo import bar
    ...
    # In the first method, we must use the foo prefix whenever we access 
    # the module bar. In the second method, we don't, because we import 
    # the module to our module's namespace.

    # The __init__.py file can also decide which modules the package exports 
    # as the API, while keeping other modules internal, by overriding 
    # the __all__ variable, like so:
__init__.py:
